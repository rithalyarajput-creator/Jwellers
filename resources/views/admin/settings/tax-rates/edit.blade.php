<x-layouts.admin>
    <x-slot name="title">Edit Tax Rate</x-slot>
    ...
    <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 1.25rem;">
        ...
        <h1 style="font-size: 1.125rem; font-weight: 600; color: #303030;">{{ $taxRate->name }}</h1>
    ...
    <form action="{{ route('admin.settings.tax-rates.update', $taxRate) }}" method="POST">
        ...
        <div class="card" style="max-width: 640px; margin-top: 1rem; padding: 1.25rem;">
            ...
            <div style="display: flex; flex-direction: column; gap: 1rem;">
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem;">
                    <div>
                        ...
                        <input type="number" name="cgst_rate" value="{{ old('cgst_rate', $taxRate->cgst_rate) }}" step="0.01" min="0" max="100" required
                               class="form-input">
                        @error('cgst_rate')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="sgst_rate" value="{{ old('sgst_rate', $taxRate->sgst_rate) }}" step="0.01" min="0" max="100" required
                               class="form-input">
                        @error('sgst_rate')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="igst_rate" value="{{ old('igst_rate', $taxRate->igst_rate) }}" step="0.01" min="0" max="100" required
                               class="form-input">
                        @error('igst_rate')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                ...
                <div style="display: flex; align-items: center; gap: 0.5rem;">
                    ...
                    <input type="checkbox" name="is_active" value="1" id="is_active"
                           ...
                           @checked(old('is_active', $taxRate->is_active))>
            ...
            <div style="display: flex; align-items: center; justify-content: space-between; margin-top: 1.25rem; padding-top: 1rem; border-top: 1px solid #e3e3e3;">
                <form action="{{ route('admin.settings.tax-rates.destroy', $taxRate) }}" method="POST"
                      onsubmit="return confirm('Delete this tax rate?')" style="display: inline;">
                    @csrf @method('DELETE')
                    <button type="submit" style="font-size: 13px; font-weight: 500; color: #d72c0d; background: none; border: none; cursor: pointer;">Delete tax rate</button>
                </form>
                <div style="display: flex; align-items: center; gap: 0.5rem;">
